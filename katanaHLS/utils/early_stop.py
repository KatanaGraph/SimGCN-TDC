# This code has been modeled after DGL Lifescience module
...
class EarlyStopping(object):
    ...
    """
    def __init__(self, mode='higher', patience=10, filename=None, metric=None, verbose=True):
        if filename is None:
        ...
        if metric is not None:
            assert metric in ['r2', 'mae', 'rmse', 'roc-auc', 'pr-auc', 'spearman'], \
                "Expect metric to be 'r2' or 'mae' or " \
                "'rmse' or 'roc_auc_score', got {}".format(metric)
            if metric in ['r2', 'roc_auc_score', 'pr_auc_score', 'spearman']:
